    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot='sidebar'
          data-mobile='true'
          className='w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden rounded-none'
          style={
            {
...
function SidebarTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        "group/trigger size-8 md:size-8.5 rounded-none border-0 p-0 bg-transparent! hover:bg-transparent! dark:hover:bg-transparent! active:bg-transparent! dark:active:bg-transparent! text-ml-text-muted hover:text-ml-text transition-colors duration-150 inline-flex items-center justify-center cursor-pointer select-none outline-none focus:outline-none shadow-none ring-0 focus-visible:ring-0 group-data-[collapsible=icon]:size-8 md:group-data-[collapsible=icon]:size-8.5 group-data-[collapsible=icon]:mx-auto",
        className,
      )}
      {...props}
    >
      <AnimatePresence mode='wait' initial={false}>
        <motion.span
          key={isCollapsed ? "closed" : "open"}
          initial={{ rotateY: 90, opacity: 0 }}
          animate={{ rotateY: 0, opacity: 1 }}
          exit={{ rotateY: -90, opacity: 0 }}
          transition={{ duration: 0.18, ease: "easeInOut" }}
          className='inline-flex items-center justify-center size-full will-change-transform'
        >
          <HugeiconsIcon
            icon={isCollapsed ? PanelLeftCloseIcon : PanelLeftOpenIcon}
            size={21}
            strokeWidth={1.5}
            className='size-5 md:size-5.5 text-ml-text-muted transition-colors duration-150 group-hover/trigger:text-ml-text'
          />
        </motion.span>
      </AnimatePresence>
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar='input'
      className={cn(
        "h-8 w-full border-input bg-muted/20 dark:bg-muted/30 rounded-none",
        className,
      )}
      {...props}
    />
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar='header'
      className={cn(
        "flex flex-col gap-2 p-2 rounded-none group-data-[collapsible=icon]:p-0",
        className,
      )}
      {...props}
    />
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar='footer'
      className={cn("flex flex-col gap-2 p-2 rounded-none", className)}
      {...props}
    />
  );
}
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn("mx-2 w-auto bg-sidebar-border rounded-none", className)}
      {...props}
    />
  );
}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-none p-2 text-left text-xs ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-0! group-data-[collapsible=icon]:gap-0 group-data-[collapsible=icon]:justify-center hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
